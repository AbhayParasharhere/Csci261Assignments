...
char *to_decimal(char *number)
{
    ...
    else if (is_decimal(number) == TRUE)
    {
        ...
        decimal_equi = strtod(numcpy, &end_ptr);
        result = itoa(decimal_equi, numcpy, RADIX_DECIMAL);
        return result;
    }
}
...
char *to_binary(char *number)
{
    ...
    {
        result = hex_to_binary(numcpy);
        return result;
    }
    else if (is_decimal(number) == TRUE)
    {
        result = decimal_to_binary(numcpy, DECIMAL_MAX_DIGITS);
...
char *from_fixed_point(char *number)
{
    ...
    if (is_fixed_point(number) == TRUE)
    {
        int_part_bin = get_integer_part(number);

        int_part = binary_to_integer_part(int_part_bin);

        frac_part_bin = get_fraction_part(number);
