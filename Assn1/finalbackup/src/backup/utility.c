...
int is_hex(char* number)
{
    ...
    if(strlen(number_no_prefix) <= 8 /*HEX_MAX_DIGITS*/ && number[0] == '0'
      && number[1] == 'x' && are_hex_digits(number_no_prefix) == 1 /*TRUE*/)
    {
...
int is_binary(char* number)
{
    ...
    if(strlen(number_no_prefix) <= 32 /*BINARY_WORD_SIZE*/ && number[0] == '0'
      && number[1] == 'b' && are_binary_digits(number_no_prefix) == 1 /*TRUE*/)
    {
        free(number_no_prefix);//Free number_no_prefix as checking is already done
        return 1; /*TRUE*/
    }
...
int is_fixed_point_part(char* number)
{
    ...
    if(size <= 16 /*FIXED_POINT_PART_MAX_DIGITS*/ && are_binary_digits(number) == 1 /*TRUE*/)
    {
        return 1; /*TRUE*/
    }
    else
...
int is_fixed_point(char* number)
{
    ...
    if(size <= 32 /*FIXED_POINT_MAX_DIGITS*/ && are_binary_digits(number) == 1 /*TRUE*/)
    {
        return 1; /*TRUE*/
    }
    else
...
char* get_integer_part(char* number) // remember to free memory of returned ptr after use
{
    ...
    char *int_part = malloc ((sizeof(char) * size)-1); // allocate sufficient memory for int_part 
